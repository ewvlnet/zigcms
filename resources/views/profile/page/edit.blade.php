<x-profile-layout>
    ...
    <div class="container-xxl flex-grow-1 container-p-y">
        <header class="col-md-6 px-0"><h4 class="fw-bold py-3"><span
                    class="text-muted fw-light">Edit /</span> {{ Str::limit($page->title, 100) }}</h4>
        </header>
        <x-shared.flash-msg/>
                ...
                <div class="card">
                    <div class="d-flex align-items-end row">
                        <div class="card-body">
                            ...
                            <form action="{{ route('profile.pages.update', $page) }}" class="form" id="formData"
                                  method="POST">
                                @csrf @method('PUT')
                                <div class="row px-3">
                                    ...
                                    <div class="mb-3 col-md-6">
                                        <x-shared.form.select label="{{__('Category')}}" name="category_id">
                                            @foreach($categories as $category)
                                                <option
                                                    value="{{ $category->id }}" {{ old('category_id', $page->category_id) == $category->id ? 'selected' : '' }} >
                                                    {{ $category->name }}
                                                </option>
                                            @endforeach
