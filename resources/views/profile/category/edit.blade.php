<x-profile-layout>
    ...
    <div class="container-xxl flex-grow-1 container-p-y">
        <header class="col-md-6 px-0"><h4 class="fw-bold py-3"><span
                    class="text-muted fw-light">Edit /</span> {{ Str::limit($category->name, 100) }}</h4>
        </header>
        <x-shared.flash-msg/>
                ...
                <div class="card">
                    <div class="d-flex align-items-end row">
                        <div class="card-body">
                            ...
                            <form action="{{ route('profile.categories.update', $category) }}" class="form"
                                ...
                                <div class="row px-3">
                                    <div class="mb-3 col-md-6">
                                        <x-shared.form.input label="{{__('Name')}}" name="name"
                                                             ...
                                                             placeholder="{{__('Name category')}}" required="on"/>
                                    </div>

                                    <div class="mb-3 col-md-6">
                                        <x-shared.form.textarea label="{{__('Category description')}}"
                                                                name="description"
                                                                value="{{ old('description', $category->description ?? '') }}"
                                                                rows="5"
                                                                placeholder="{{__('Category short description')}}"/>
                                    </div>
                                </div>
