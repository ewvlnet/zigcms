<x-site-layout>
    <x-slot name="title">Category:. {{ $category->name }}</x-slot>
    <x-slot name="hero"><x-site.home.slider/></x-slot>

    <section class="home-articles row mb-2">
        <header class="col-md-6 px-0"><h2 class="display-4 fst-italic">{{ $category->name }}</h2></header>
        <p><small>Category: {{ $category->name }}</small> {!! $category->description ?? '&#8734;' !!}</p>
